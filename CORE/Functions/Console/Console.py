...
#> reports load
def load_reports():
    if os.path.exists(DataTechStatic.REPORTS_FILE):
        with open(DataTechStatic.REPORTS_FILE, 'r') as f:
            return json.load(f)
    return {}


def save_reports(reports):
    with open(DataTechStatic.REPORTS_FILE, 'w') as f:
...
def print_server_ip():
    try:
        ...
        print_success(f"Server IP address: {ip_address}:{DataTechStatic.port}")
...
def get_ukrainian_badges(user):
    return [DataStrStatic.BADGES.get(badge, badge) for badge in user.get('badges', [])]
...
#>users list
def load_users():
    if not os.path.exists(DataTechStatic.USERS_FILE):
        return {}

    try:
        with open(DataTechStatic.USERS_FILE, 'r', encoding='utf-8') as f:
...
def save_users(users):
    try:
        with open(DataTechStatic.USERS_FILE, 'w', encoding='utf-8') as f:
...
#>forums list
def load_forums():
    if os.path.exists(DataTechStatic.FORUMS_FILE):
        with open(DataTechStatic.FORUMS_FILE, 'r') as f:
            return json.load(f)
    return {}

def save_forums(forums):
    with open(DataTechStatic.FORUMS_FILE, 'w') as f:
...
def assign_badge_to_user(uid, badge_name):
    ...
    user = next((u for u in users.values() if u['uid'] == uid), None)
    if user:
        if badge_name in DataStrStatic.BADGES:
            assign_badge(user, badge_name)
            save_users(users)
            print(f"Badge '{DataStrStatic.BADGES[badge_name]}' has been assigned to user {user['username']} ({uid})")
        else:
